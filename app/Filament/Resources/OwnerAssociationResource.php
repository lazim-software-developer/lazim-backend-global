...
class OwnerAssociationResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        ...
        $modules = Module::all();
         $moduleComponents = $modules->isNotEmpty()
        ? $modules->map(fn ($module) =>
            Toggle::make("modules.{$module->id}")
                ->label($module->name)
        )->all() // return array of Toggle components
        : [
            Placeholder::make('no_modules')
                ->label('Modules')
                ->content('No modules exist.')
        ];
        return $form
            ->schema([
                ...
                Section::make('Modules')
                    ->columns(3)
                    ->schema($moduleComponents), // pass directly
            ]);
    }
    // public static function form(Form $form): Form
    // {
